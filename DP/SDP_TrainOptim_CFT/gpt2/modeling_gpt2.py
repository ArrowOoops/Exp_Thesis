...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        private_positions: Optional[torch.LongTensor] = None,
        ...
        if labels is not None:
            if private_positions is not None:
                ...
                # 将隐私token损失与非隐私token损失分离
                mask = torch.zeros_like(losses, dtype=torch.bool)
                mask[private_positions] = True
                private_losses = losses[mask]
                non_private_losses = losses[~mask]

                # 在隐私token损失上添加高斯噪声
                private_losses += torch.normal(0, sigma, size=private_losses.size(), device=private_losses.device)

                # 计算总损失
                total_loss = torch.mean(torch.cat((private_losses, non_private_losses)))
