...
# def create_digit_mask(tokenizer, lm_logits):
#     digit_indices = []
#     for idx, token in enumerate(tokenizer.get_vocab()):
#         if token.isdigit():
#             digit_indices.append(idx)

#     mask = torch.zeros_like(lm_logits)
...
def create_digit_mask(tokenizer, lm_logits):
    ...
    #     print("tokenizer.decode(item) = ", tokenizer.decode(item))

    batch_size, vocab_size = lm_logits.size()
    mask = torch.zeros(batch_size, vocab_size).to(lm_logits.device)
    for idx in digit_indices:
        mask[:, idx] = 1
    # print("mask = ", mask)
    return mask
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        last_token_logits = lm_logits

        # print("last_token_logits.shape = ", last_token_logits.shape)

        upper_bound = last_token_logits.max()
        lower_bound = last_token_logits.min()
        last_token_logits = (last_token_logits - lower_bound) / (
        ...
        if sigma is not None:
            digit_mask = create_digit_mask(tokenizer, last_token_logits)
            noise = torch.normal(mean=0, std=sigma, size=last_token_logits.shape).to(
                last_token_logits.device
            )
            noise *= digit_mask  # Apply the mask to the noise

            noisy_logits = last_token_logits + noise
            lm_logits = noisy_logits
